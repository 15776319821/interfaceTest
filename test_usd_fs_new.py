...
def monitor_exchange_rate(app_id, threshold, target_currencies, feishu_webhook_url, days_to_compare=7, check_interval=86400):
    ...
    for currency in target_currencies:
        ...
        if abs(change) >= threshold:
            message = f"汇率变化超过阈值! {base_currency} -> {currency}:  {change * 100:.2f}% {today}={current_rate} -- {compare_date}={prev_rate}"
            print(message)
            send_feishu_message(feishu_webhook_url, message)

if __name__ == "__main__":
    YOUR_APP_ID = "81df0e7f2aa54f94a6c5ae2c4f04f119"  # 替换为你的APP ID
    TARGET_CURRENCIES =["AED", "BHD", "EGP", "IRR", "IQD", "JOD", "KWD", "LBP", "LYD", "MAD", "OMR", "PKR", "QAR", "SAR", "TND", "YER", "ARS", "BOB", "BRL", "CLP", "COP",  "EUR", "GYD", "PYG", "PEN", "SRD", "UYU", "BND", "MMK", "KHR", "IDR", "LAK", "MYR", "PHP", "SGD", "THB", "VND", "USD", "CAD", "MXN", "EUR", "BGN", "HRK", "CZK", "DKK", "HUF", "ISK", "NOK", "PLN", "RON", "SEK", "CHF", "GBP"]  # 要监控的货币列表，例如：["CNY", "EUR", "JPY"]
    THRESHOLD = 0.03  # 变化阈值，例如：0.1 表示 10% 变化
